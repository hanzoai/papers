...
\paragraph{Where the cost actually is.} Driving the identical operation
against the filer's own HTTP API on a reused connection---bypassing FUSE
entirely---creates files in \textbf{8.36\,ms}, or 120/s. The same create
through the mount costs \(\approx 37\,\)ms. \textbf{The projection layer
accounts for roughly \(4.4\times\) of the cost; the store accounts for the
rest.} This matters for where effort should go: the mount, not the object
store, is the first thing to optimise, and the store's own 8.36\,ms create is
a second, separate question (we did not determine whether it is
\texttt{fsync}-bound).

We attempted one tuning experiment---enabling the driver's file-chunk read
cache, which ships disabled---and report it as inconclusive rather than as a
result: the knob caches \emph{chunks} for reads while this workload is bound
by \emph{metadata} operations, our two runs did not time identical scopes,
and mount daemons restarted mid-measurement. A serious tuning pass remains
open work, and nothing here should be read as establishing a floor.
...
compensates for that. Whether the per-link cost is 37\,ms or 4\,ms decides
whether a workload like Git is impossible or merely expensive---so the number
has to be measured, per layer, before an architecture is built on an
assumption about it.
