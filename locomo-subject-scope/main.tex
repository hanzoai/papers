...
\date{Measurement pass 2026-09-09 \quad \texttt{hanzoai/semantic} v0.2.3}
...
questions and four systems. Three results follow, and the negative controls are the
...
which beats every system we ran. A quarter of LoCoMo's questions are
adversarial and the official metric awards a point for declining them, so any
mean over the full set measures the abstention rate before it measures the
memory. The answerable and adversarial halves have to be reported apart.
...
adversarial questions (95\% CI over conversations $[+0.247, +0.346]$) at no
measurable cost on the 1{,}540 answerable ones ($0.142$ against $0.144$, CI
$[-0.011, +0.006]$). The mechanism is reader-independent: with the result limit
lifted past the size of the conversation, the scoped index can reach $76\%$ of
...
adversarial questions at AUC $0.514$, which is chance; the scoped index's
...
which derived 44 facts from 23{,}942 extracted claims, moves nothing at three
decimals. A $2\times2$ over what is indexed against what a query may reach
attributes $+0.152$ of the adversarial gain to subject scope alone, $-0.030$ to
the claim-span unit alone, and $+0.180$ to their interaction. An oracle handed
the dataset's own annotated evidence scores \emph{worse} on adversarial
questions than the scoped index does ($0.426$ against $0.502$), because on those
...
\item The inference step is inert. Removing it changes nothing
...
was present, asked about it, and never claimed it. The reference answer is that
the conversation does not say. LoCoMo's construction here follows the
...
writes. Every figure in this paper agrees between the two to three decimals,
across 1{,}986 questions and four systems. The original scorer is fetched at
test time rather than vendored, because LoCoMo is CC BY-NC 4.0 and our
repository is MIT.
...
\textbf{Silence} is how often a system declined to answer.
...
conversations, the span of words it may take moves the scoped index's overall
...
Resolving the subject of a \emph{question} is easier: 1{,}964 of the 1{,}986
LoCoMo questions ($98.9\%$) name one of the two speakers, and the longest known name the question
contains is taken as its subject. The remaining $1.1\%$ name nobody and fall
...
questions four ways costs 5.6\,s wall, 5.0\,s user and 254\,MB peak resident on
...
$0.228$ beats every system in this paper on the same aggregate. It beats the
best of them, which scores $0.223$, and the oracle handed perfect evidence,
which scores $0.252$, is only $0.024$ above a system with no memory at all.
...
two rows that matter. On the 1{,}540 answerable questions the scoped and flat
indexes are level, $0.142$ against $0.144$. On the 446 adversarial ones they
separate, $0.502$ against $0.200$.
...
report the percentile bootstrap~\cite{bootstrap} over 10{,}000 resamples of the
ten conversations with replacement.

\begin{table}[t]
\centering
\small
\caption{Paired differences against \arm{vector}, with 95\% percentile bootstrap
intervals over the ten conversations (10{,}000 resamples). The adversarial gain
...
\midrule
\arm{graph}  $-$ \arm{vector} & $+0.303$ & $[+0.247, +0.346]$ & $-0.002$ & $[-0.011, +0.006]$ \\
\arm{scoped} $-$ \arm{vector} & $+0.152$ & $[+0.107, +0.189]$ & $-0.000$ & $[-0.007, +0.007]$ \\
\arm{span}   $-$ \arm{vector} & $-0.029$ & $[-0.058, -0.004]$ & $+0.007$ & $[-0.000, +0.017]$ \\
\arm{graph}  $-$ \arm{scoped} & $+0.150$ & $[+0.107, +0.190]$ & $-0.002$ & $[-0.008, +0.004]$ \\
\arm{oracle} $-$ \arm{vector} & $+0.226$ & $[+0.160, +0.276]$ & $+0.058$ & $[+0.048, +0.069]$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{One question, four answers.} The separation is legible in a single
case, from \texttt{conv-43}. Turn \texttt{D20:2} is John saying \emph{``Congrats
on your success! I'm also trying out yoga to get a little extra strength and
flexibility.''} The question is \emph{``What is Tim trying out to improve his
strength and flexibility after recovery from ankle injury?''} John does yoga;
Tim asked him about it; the reference answer is that the conversation does not
say.
...
and the flat index is right about that and wrong about the question. The scoped
index never sees \texttt{D20:2}, because the sentence carrying \emph{yoga} has
John as its subject and the span was filed under John. Asked about Tim, it finds
nothing above the floor and says so. The oracle answers wrongly for the same
reason as the flat index, from better evidence --- see \S\ref{sec:oracle}.
...
\emph{Measured.} At $k=2000$, the flat turn index reaches $0.992$ of the
annotated evidence for answerable questions and $0.964$ for adversarial ones.
The claim-span index with the filter removed reaches $0.863$ and $0.916$. Adding
the subject restriction changes the second number and not much else: whole turns
scoped by person reach $0.885$ and $0.350$; claim spans scoped by person reach
$0.756$ and $0.269$.

\input{figures/reach}

\emph{Interpretation.} The subject restriction makes the index selectively
blind, and the blindness is concentrated where the trap turns are. It costs
$11$--$24$ points of answerable evidence, which is the price of a rule-based
extractor: a sentence whose subject it fails to resolve produces no span for
that person, and the turn becomes unreachable through them. It removes $61$--$70$
points of adversarial evidence, which is the point, because an adversarial
question's annotated evidence is a turn about somebody else.
...
threshold is arguable by construction. In Table~\ref{tab:main} the scoped index
is silent on $31\%$ of questions and the flat one on $20\%$. Two measurements
...
the signal. Both scoped indexes are above chance, and the AUC tracks the scope
factor, not the unit factor: changing the unit alone moves it by $0.000$,
adding the scope moves it by $0.121$ on turns and $0.184$ on spans. The oracle is
also at chance. Perfect retrieval confers no ability to tell an answerable
...
answerable score the scoped indexes are far to the right of the flat ones.
...
At every matched silence rate the scoped index leads on both halves. Either
system can be made as quiet as one likes; only one of them falls silent on the
right questions.
...
questions it scores $0.426$, \emph{below} the scoped index's $0.502$, and with
abstention off it scores $0.045$, below even the flat index's $0.092$.
...
\textbf{Result:} every figure in Table~\ref{tab:main} is byte-identical.
...
\textbf{Result:} single-hop F1 moves from $0.121$ to $0.122$ and its reach from
$0.507$ to $0.505$; answerable F1 is $0.142$ either way; the aggregate is
$0.223$ either way. Every other cell is unchanged at three decimals. The one
digit that moves, moves in favour of removing the inference.
...
node, and that requires no inference at all. Once identity is resolved there is
very little for a reasoner to add. This says where the value in a memory graph
sits, and it is not in the deductive layer.
...
in F1; the interaction is
$(\arm{graph}-\arm{scoped})-(\arm{span}-\arm{vector})$.}
\label{tab:factorial}
...
\multicolumn{6}{@{}l}{\quad claim span, with everything reachable \hfill $-0.030$} \\
...
\multicolumn{6}{@{}l}{\quad claim span \hfill $+0.060$} \\
\multicolumn{6}{@{}l}{\quad interaction \hfill $+0.180$} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Subject scope alone accounts for half the effect.} Adding nothing but
the subject partition to the unchanged turn index takes the adversarial score
from $0.200$ to $0.352$, a gain of $+0.152$ against a total of $+0.302$, with
no movement on the answerable half ($0.144$ to $0.144$). No claim extraction is
needed to index by subject --- the extractor is used here only to decide which
people a turn is about --- and no graph is needed at all.
...
\textbf{The largest single term is the interaction.} $+0.180$ of the $+0.302$
...
while a claim span belongs to exactly one person. The two changes are not
...
one node's several edges. Retrieval bears that out slightly: multi-hop reach is
$0.228$ against the flat index's $0.194$ at $k=5$, and $0.419$ against $0.388$
under settings that let an answer be a list. F1 does not follow: $0.020$ against
...
$k$ the scoped index reaches $0.688$ of multi-hop evidence where the flat index
reaches $0.985$. Subject scope improves multi-hop \emph{ranking} at small $k$
...
for, with lexical weighting~\cite{bm25} substituted for a learned embedding for
the reasons given in \S\ref{sec:indexes}. Graph-structured and
...
\textbf{A rule-based extractor.} The $11$--$24$ points of answerable reach the
scoped index gives up are the extractor's failures, not a property of subject
scoping. A model-based extractor would move that cost and we do not know by how
much.
...
MIT, tag \texttt{v0.2.3}. It is Go with no dependencies outside the standard
...
Every number in this paper comes from one of:
...
\midrule
\texttt{go run ./bench/locomo} & Table~\ref{tab:main} \\
\texttt{go run ./bench/locomo -factor} & Table~\ref{tab:factorial} \\
\texttt{go run ./bench/locomo -sweep floor} & Figure~\ref{fig:frontier}, \S\ref{sec:abstention} \\
\texttt{go run ./bench/locomo -k 2000 -floor 0} & Figure~\ref{fig:reach} \\
\texttt{go run ./bench/locomo -out <file>} & the per-question record \\
\texttt{go test ./bench/locomo/} & the metric port, three ways \\
\texttt{make check} \emph{(in \texttt{bench/locomo})} & the same predictions, scored by the original Python \\
\bottomrule
\end{tabular}
\end{center}

The predictions file carries each system's reply, its retrieved turn
identifiers, and its confidence, so the AUCs of \S\ref{sec:abstention} and the
bootstrap intervals of Table~\ref{tab:ci} are computed from a published artifact
rather than asserted. The two controls of \S\ref{sec:control-graph} and
\S\ref{sec:control-reason} are single-line edits to \texttt{graph.go}: build the
graph from the empty fact set, and skip the loop over derived facts.
...
Constant abstention scores $0.228$ and beats every system we built. Any mean
...
derived 44 facts from 23{,}942, moves nothing at three decimals. Neither of
those is a defect in the implementation; both say where the value in this kind
of memory is not.
...
partition unambiguous. An oracle with perfect retrieval does worse than the
scoped index on those questions, because there the right turn is the wrong
answer.
