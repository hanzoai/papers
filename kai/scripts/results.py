...
Keys:
  ...
  kj/<suite>/<who>/<metric>   who: kai, laya (kai-1 as Laya's router picks), jev, and the
                              kai-1 checkpoints one, multi, agent; suite as in the harness,
  ...
  td/<ckpt>/...               upstream's typed-decisions benchmark rerun on kai-1
  par/...                     native kai-1 against the reference runtime
...
# ------------------------------------------------------------------ the frozen harness
# scores.json's backends: Kai, kai-1 routed as Laya's router picks, each kai-1 checkpoint, Jev
...
def td(runs):
    """Upstream's Part B (typed decisions) on the three kai-1 checkpoints."""
    d = load(runs["td"])
    if d is None:
        return
...
# ------------------------------------------------------------------ capability suites
def capability(runs):
    """decision/capability/results/<suite>.json: {"meta", "keys": {cap/<suite>/<who>/<metric>}},
    each key written as the file names it."""
    root = path(runs["cap"])
    if not os.path.isdir(root):
        print("%% absent: %s" % runs["cap"])
        return
...
def main():
    ...
    for part in (harness, overlap, td, parity, capability):
        part(runs)
