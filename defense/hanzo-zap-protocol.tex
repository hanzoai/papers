...
\textbf{ZAP} & \textbf{2.2 ns} & \textbf{0} & \textbf{256} & \textbf{None} \\
\bottomrule
\end{tabular}
...
\begin{enumerate}
\item \textbf{Zero-copy wire format}: 16-byte header, 8-byte aligned structs, direct buffer reads at 2.2~ns.
...
\item \textbf{Go}: Used in Hanzo consensus and VM communication.
...
\begin{enumerate}
\item \textbf{Post-quantum crypto FFI library} (preferred): FFI binding to the Hanzo cryptographic library (CIRCL-based, CGO-optimized). Shared with the consensus layer.
\item \textbf{pqcrypto} (fallback): Pure Rust PQ implementations for environments without the native crypto library.
...
\textbf{Note}: The Go and Rust implementations benchmark different operations. The Go ``Parse'' (2.2~ns) reads a pre-built message from buffer. The Rust ``Serialize'' (322~ns) constructs a new message. The Rust ``Deserialize'' (21~ns) performs schema-validated parsing, more expensive than raw buffer validation.
...
\midrule
\texttt{\_hzd.\_tcp} & Blockchain consensus nodes \\
\texttt{\_hz-gpu.\_tcp} & GPU compute nodes \\
\texttt{\_hz-vm.\_tcp} & Virtual machine endpoints \\
...
\item \textbf{did:hanzo}: Anchored to the Hanzo chain. Verifiable via consensus.
...
% ============================================================================
\section{Corona Consensus Integration}
% ============================================================================
...
\item \textbf{Performance}: $\sim$89~ms for threshold signature (within 3-second quantum bundle window).
\end{itemize}
...
\texttt{consensus.rs} & 35.5 KB & Corona threshold signing \\
...
\item Single external dependency: \texttt{github.com/hanzoai/mdns}.
\end{itemize}
...
\bibitem{corona} ``Practical Two-Round Threshold Signature from LWE,'' IACR ePrint 2024/1113, 2024.
...
\bibitem{luxzap} Hanzo Team, ``ZAP: Consensus Wire Protocol,'' Hanzo, 2026.

\end{thebibliography}

\section*{Reproducibility}
\textbf{Source code.} \texttt{github.com/hanzoai/zap} (commit \texttt{TBD}).\\
\textbf{Build.} cargo build --release (Rust impl); go build ./... (Go impl).\\
\textbf{Hardware tested.} Apple M-series, x86\_64 Linux + NVIDIA A100, ARM Cortex-A (edge).\\
\textbf{Standards referenced.} Cap'n Proto wire spec, ML-KEM-768, ML-DSA-65 (FIPS 203/204), W3C DID.\\
