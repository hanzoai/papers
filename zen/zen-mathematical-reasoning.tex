...
\author{Antje Worring, Hanzo AI Research, Zen LM Research Team\\
...
supervision, and a symbolic-neural hybrid verification system. Zen-7B scores 71.4\%
on the MATH benchmark, 95.8\% on GSM8K, 38.3\% on AIME 2024, 74.2\% on AMC 2023,
and 28.1\% on FrontierMath; Zen-32B with the step-level PRM scores 83.4, 97.9, 48.2,
84.7, and 36.9 on the same five. The PRM adds 4.8 MATH points at 7B (71.4 to 76.2)
and 4.6 at 32B (78.8 to 83.4) over the same model without it. The hybrid
verification system using SymPy for
...
\bottomrule
\end{tabular}
\caption{Mathematical reasoning benchmark results. The PRM adds between 0.7 and 4.8
points, depending on benchmark and model size. AIME 2024 is 30 problems, so a
single-pass score is a multiple of 3.3 points; these AIME figures are therefore
averages over repeated samples. The sample count is on record only for
Zen-32B + PRM ($n=128$, Section~\ref{sec:olympiad}); for the other three rows it
is not.}
\end{table}

Each row is one decode per problem on MATH and GSM8K. What test-time sampling adds
is reported separately in Table~\ref{tab:sampling}: Zen-7B with PRM reranking reaches
78.1\% MATH at $k=32$ and 79.0\% at $k=64$.
...
step-level guidance (+4.6 to +4.8 MATH points), and hybrid verification enabling
reliable self-evaluation. The combination yields 83.4\% MATH, 97.9\% GSM8K, and
48.2\% AIME for Zen-32B with the PRM. Open problems include proof-level verification for advanced mathematics
