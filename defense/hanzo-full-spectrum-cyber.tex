...
\State Each validator computes Corona share (Round 1)
...
\item \textbf{GPU acceleration}: NTT/iNTT, polynomial multiplication, and TFHE bootstrap operations accelerated via the Hanzo GPU kernel library's Metal/CUDA kernels.
...
The stable C ABI (\texttt{hz-accel/c\_api.h}) enables FFI bindings from Go, Rust, and Python.
...
\textbf{Source code.} \texttt{github.com/hanzoai/agents, hanzoai/zap} (commit \texttt{TBD}).\\
\textbf{Build.} make build per component; OpenZiti overlay deployed via Helm.\\
\textbf{Hardware tested.} Linux amd64/arm64; tested in air-gapped lab environments.\\
\textbf{Standards referenced.} OpenZiti, NIST PQC, MITRE ATT\&CK.\\
