...
\author{
    Hanzo AI Research\thanks{research@lux.network} \\
...
VQA v2 is not reported. It scores natural-language answers about an input image,
and this model takes text and returns pixels.
...
A diffusion sampler emits images rather than tokens, so neither a token rate nor a
time-to-first-token applies here; the corresponding measure is seconds per image at
the sampler's step count.
...
\begin{itemize}
    \item \textbf{Carbon Footprint}: 0.09 kg CO\textsubscript{2}e per million inferences
