...
\begin{table}[H]
\centering
\caption{Inference throughput across hardware targets (4-bit quantized). Batch size
is not recorded, and the ms/tok column is the reciprocal of the tokens/second column
rather than a separate measurement. One decode stream reads the resident weights once
per token, which on the 4090 row (950 MB, 1{,}008 GB/s) bounds a single stream at about
1{,}060 tokens per second.}
\begin{tabular}{lcccc}
\toprule
