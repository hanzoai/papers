...
\begin{abstract}
We present Hanzo IAM, a multi-tenant identity and access management platform that serves as the unified authentication layer for the Hanzo ecosystem. IAM supports OAuth~2.0, OpenID Connect, SAML~2.0, LDAP, and WebAuthn protocols, integrating with over 40 social identity providers. The system implements Casbin-based role-based access control (RBAC) with per-organization policy engines, SCIM~2.0 provisioning for enterprise directory synchronization, and TOTP/WebAuthn multi-factor authentication. This paper describes the protocol surface, the policy model and the credential handling, and reports no measurements; Section~\ref{sec:figures} records the request volume, tenant count and authentication latency an earlier version reported and why they are withdrawn. All credentials are stored using Argon2id hashing with per-tenant salt derivation, and session tokens employ cryptographic binding to prevent token theft.
\end{abstract}
...
Policies support role hierarchies, resource wildcards, and time-based constraints. Policy decisions are served from an in-memory cache with write-through invalidation, so an evaluation is a map lookup rather than a datastore round trip; the cost of that lookup is not measured here (Section~\ref{sec:figures}).
...
An earlier version of this paper carried a table titled ``Authentication latency
(ms) by protocol'' giving p50/p99 of 12/28\,ms for OIDC token issue, 0.3/0.8\,ms
for JWT validation, 18/45\,ms for SAML assertion, 8/22\,ms for WebAuthn verify
and 0.05/0.1\,ms for an RBAC check. The abstract restated these as a median
authentication latency of 18\,ms over more than two million requests daily
across 1{,}200 or more tenant organizations, and the policy section stated that
evaluation adds less than 0.1\,ms.

None of it was measured. The table named no hardware, no load generator and no
request count; \texttt{hanzoai/iam} contains no benchmark function and no load
test; and no deployment record in this estate counts authentication requests or
tenant organizations. The daily-volume and tenant figures in particular describe
an operating history the service does not have, so they cannot be relabelled as
targets and are removed.

The latency figures are the cheapest of these to establish honestly, because
each protocol path is a function call away from a Go benchmark in the same
repository. What is missing is the run and the machine it ran on, reported the
way the Hanzo cloud data plane benchmark~\cite{clouddataplane} reports one.
...
By 2019 the limitations were clear: organizations wanted SSO with their existing directories (Active Directory, Okta), compliance teams wanted audit logs with tamper evidence, and mobile clients wanted passwordless flows. The current IAM platform (2020--) is a ground-up rewrite, with the KrakenD gateway retained as a complementary rate-limiting and routing layer. The Casbin policy engine replaces the ad-hoc role checks of the 2016 era; the audit trail described in Section~\ref{sec:security} generalizes what was previously a coarse access log. The 40+ social provider support, SCIM~2.0 provisioning, and hardware-backed authentication are all post-2020 additions.

\section{Conclusion}

Hanzo IAM is a unified identity platform that combines authentication, authorization, and user management in a single service. By supporting the major identity protocols and 40+ social providers, IAM removes the need to run several identity systems side by side. The Casbin-based policy engine provides fine-grained access control, and native SCIM provisioning enables enterprise directory integration. This paper reports no latency or volume measurements; Section~\ref{sec:figures} says which were withdrawn and what would replace them.
