...
\begin{enumerate}
\item \textbf{RNS-Native Consensus}: Integration of the Reticulum Network Stack as a first-class transport layer in the Hanzo consensus protocol, enabling validator communication over any physical medium.
...
\bottomrule
\end{tabular}
\caption{Unified endpoint addressing in Hanzo consensus.}
\label{tab:endpoints}
\end{table}
...
\begin{lstlisting}[language=Go,caption={RNS transport configuration in the Hanzo node.}]
...
\item \textbf{Storage}: Persistent volumes on block storage for stateful services (PostgreSQL, NATS JetStream, Datastore).
...
\begin{itemize}
\item RNS transport fully integrated into Hanzo consensus with hybrid PQ handshake.
...
\bibitem{luxconsensus} Hanzo Team, ``Hanzo Consensus: Multi-Protocol Byzantine Agreement with Post-Quantum Finality,'' Hanzo, 2025.

\bibitem{quasar} Hanzo Team, ``Quasar: Hybrid BLS-Corona Consensus with Sub-Second Quantum Finality,'' Hanzo, 2025.
...
\textbf{Build.} cargo build --release (Rust 1.78+); Go binaries via make build.\\
\textbf{Hardware tested.} Apple M-series, Linux amd64/arm64 (edge: ARM Cortex-A), tested with mDNS multicast.\\
\textbf{Standards referenced.} ML-KEM-768, ML-DSA-65 (FIPS 203/204), W3C DID, DOD DDIL.\\
