...
\textbf{Source code.} \texttt{github.com/hanzoai/agents} (commit \texttt{TBD}).\\
\textbf{Build.} \texttt{pnpm install \&\& pnpm build} (Node 20+); harness extensions in \texttt{python/hanzo\_harness} via \texttt{uv sync}.\\
\textbf{Hardware tested.} Apple M-series (M2 Pro/Ultra), x86\_64 Linux with NVIDIA A100/H100.\\
\textbf{Standards referenced.} W3C DID, OpenAI tool-use API, Model Context Protocol (MCP).\\
