...
\section{Status of the operating figures}
\label{sec:figures}

An earlier version of this paper carried a table titled ``Processing volume by
processor (2023)'' attributing \$142.3M at 99.98\% success to Stripe, \$58.7M to
Adyen, \$22.1M to PayPal, \$12.4M to a crypto channel and \$4.8M to Square. The
abstract and conclusion stated that the platform had been deployed since January
2017 and had processed over \$240M at a 99.97\% success rate with zero
double-charge incidents.

None of it has a source. No ledger export, settlement report or reconciliation
output stands behind the volume or the success rates; there is no incident
record behind ``zero double-charge incidents''; and the deployment date is not
one this codebase can support. Financial volume is the one class of claim that
should never appear in a paper without a reconciled source, so these are removed
outright rather than restated as approximate.

Nothing here is hard to establish for a system that is actually processing
payments: the double-entry ledger the paper describes is itself the source, and
a period, a currency and a reconciliation run would produce every figure above
...
Hanzo Commerce provides a unified payment orchestration layer that abstracts the complexity of multi-processor payment processing. Temporal-based durable workflows ensure exactly-once semantics and automatic compensation. Routing selects among processors on success rate, latency and cost. The argument for orchestration over a direct processor integration is structural --- a failed authorization at one processor can be retried at another under the same idempotency key, which a single integration cannot do --- and this paper does not quantify what that is worth (Section~\ref{sec:figures}).
