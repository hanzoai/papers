...
Quasar consensus for coordination. Crucially, that does not make it centralized:
S-Chain is itself permissionless public-goods infrastructure
(Section~\ref{sec:economics}), run by an open, bonded node set with no central
leader, so Datastore inherits that posture --- it runs wherever S-Chain runs,
operated by whoever runs the nodes (including, e.g., LLM-consensus quorums),
rather than by us. Not its own chain, but not centralized either. We report a
working proof-of-concept for the foundational layer and the validated
