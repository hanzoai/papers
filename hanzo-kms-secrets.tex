...
An earlier version of this paper carried a table titled ``Secret retrieval
latency (ms)'' giving p50/p99 of 3.2/8.1\,ms to fetch one secret, 12.4/28.3\,ms
for a project's secrets, 8.7/22.1\,ms to create or update one and
450/1{,}200\,ms to rotate a credential, and an abstract reporting more than
12{,}000 secrets across 180 projects with zero plaintext exposure since
deployment.

None of it was measured. There is no benchmark function or load test in
\texttt{hanzoai/kms}, and the table named no hardware, no datastore
configuration and no sample count. The inventory figures count nothing that is
recorded anywhere, and ``zero plaintext exposure since deployment'' is a
security claim that would have to rest on an audit-log review; no such review
exists. All are removed.

Retrieval latency is worth establishing here for a specific reason: KMS sits on
the start-up path of every service that needs a secret, so its p99 is somebody
else's cold start. That makes it a number to measure rather than to assert.
