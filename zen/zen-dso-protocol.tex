...
heterogeneous Zen MoDE model variants. DSO achieves 99.4\% of centralized training
quality at 64 fault-free nodes, and reduces inter-node communication bandwidth by
99.8\% or better across model sizes.
\end{abstract}
...
\begin{enumerate}
    ...
    \item \textbf{Communication Efficiency}: semantic gradient summaries are compressed
          using sketching and quantization, achieving 99.8\% bandwidth reduction.
\end{enumerate}
...
\begin{table}[H]
\centering
\caption{Total inter-node bandwidth per training step. DSO reduces bandwidth by
99.8\% or better vs.\ parameter-server all-reduce, rising with model size.}
\begin{tabular}{lrrrr}
\toprule
...
\begin{itemize}
    \item 91.8--99.8\% of centralized training quality at 64--256 nodes,
          the low end at the maximum tolerated Byzantine fraction.
