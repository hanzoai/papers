...
\subsection{Hanzo GPU Kernel Library}
...
\bottomrule
\end{tabular}
\caption{Hanzo GPU kernel categories.}
\label{tab:gpu-kernels}
\end{table}

\subsection{Unified C API}

\begin{lstlisting}[language=C,caption={GPU kernel acceleration C API (excerpt).}]
// Session management
hz_session_t* hz_create_session(hz_device_t dev);

// Tensor operations
hz_tensor_t* hz_matmul(hz_session_t* s,
    hz_tensor_t* a, hz_tensor_t* b);
hz_tensor_t* hz_attention(hz_session_t* s,
    hz_tensor_t* q, hz_tensor_t* k,
    hz_tensor_t* v, float scale);

// Crypto operations
hz_tensor_t* hz_mldsa_verify(hz_session_t* s,
    hz_tensor_t* pk, hz_tensor_t* msg,
    hz_tensor_t* sig);
\end{lstlisting}
...
ZAP's mDNS discovery (\texttt{\_hz-gpu.\_tcp}) enables automatic GPU node discovery for distributed inference and FHE operations. Direct zero-copy ciphertext handling enables GPU-to-GPU encrypted computation pipelines.
...
\textbf{Source code.} \texttt{github.com/hanzoai/engine, hanzoai/jin} (commit \texttt{TBD}).\\
\textbf{Build.} cargo build --release; LanceDB embedded by default.\\
\textbf{Hardware tested.} Apple M-series (Metal), NVIDIA Jetson, x86\_64 Linux + CUDA.\\
\textbf{Standards referenced.} OpenTelemetry, ONNX Runtime, OpenAI API.\\
