...
Ten conversations, 5,882 turns, 282 multi-hop and 841 single-hop questions
with evidence. Turns and questions are embedded with zen-embedding-0.6b;
the dataset's observation annotations serve as the fact layer for this
experiment, each observation citing the turns it was extracted from.
Retrieval is cosine over turns for the baseline and the shipped pool and
re-rank for the linked rows. Nothing is learned.
...
\begin{tabular}{lrrr}
\toprule
multi-hop ALL, linked & $k=5$ & $k=10$ & $k=20$ \\
\midrule
 & 13.5 & 23.4 & 36.2 \\
\bottomrule
\end{tabular}
...
\begin{tabular}{lrr}
\toprule
multi-hop, R@20 & ALL & ANY \\
\midrule
cosine over turns (baseline) & 22.7 & 80.1 \\
+ fact index & \ldots & \ldots \\ % TODO run locomo-retrieval-all-ablation
+ adjacency & \ldots & \ldots \\ % TODO run locomo-retrieval-all-ablation
+ time cue (shipped) & 36.2 & 87.2 \\
+ surface-entity hop & $-2$ vs shipped & \\
global reciprocal-rank fusion & $-4$ vs shipped & \\
\bottomrule
\end{tabular}
\caption{Ablation. The fact index carries most of the gain; adjacency and
the time cue are small and positive; a hop through capitalised words shared
between nearby facts and a fusion of whole ranked lists are both negative.}
\label{tab:ablation}
\end{table*}
...
The benchmark's own protocol: all-MiniLM-L6-v2 embeddings, $k=10$,
gemma-4-31B-it at temperature 0, the benchmark's judge. Reference points
from the paper \cite{locomoconv}: without rewriting, Recall@10 on dialog
queries spans 0.531--0.659 across the five systems, with AnchorMem at 0.659;
on implicit queries 0.308--0.456, with mem0 at 0.456 and Memora at 0.445;
multi-facet rewriting lifts AnchorMem's implicit recall from 0.368 to
0.524.
...
\footnotesize
\input{tables/conv}
\caption{LoCoMo-Conv test split. Rows add one mechanism each; the last is
the full read path of Figure~\ref{fig:read}.}
\label{tab:conv}
\end{table*}
