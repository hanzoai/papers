...
more than doubles it to 203k), not a property of the wire, which is strictly
tighter than HTTP text. We release the framework as \texttt{hanzoai/benchmarks}
...
zero-allocation. The ZAP wire is strictly tighter than HTTP text (fewer bytes,
no header re-parsing), so a zero-allocation codec --- pooled frame buffers,
direct header bytes --- is expected to match or beat HTTP, and to widen its lead
on a saturated link where bytes-per-request dominate. That optimization is the
subject of ongoing work in \texttt{zap-proto/http} and \texttt{zap-proto/bench}.
