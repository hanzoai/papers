...
\author{
  Hanzo AI Research \and Hanzo AI Inc \and Zoo Labs Foundation \\[0.4em]
...
\begin{itemize}[leftmargin=*,noitemsep]
  ...
  \item Persian-specific characters: \textit{peh} (U+067E), \textit{cheh} (U+0686),
        \textit{zheh} (U+0698), \textit{gaf} (U+06AF) normalized to canonical forms.
...
\State For each morpheme boundary, insert virtual space marker \texttt{\textvisiblespace{}M}
\State Apply standard BPE merge operations, treating \texttt{\textvisiblespace{}M} as protected
\State \textbf{Constraint}: Merges cannot cross \texttt{\textvisiblespace{}M} boundaries
\State Remove \texttt{\textvisiblespace{}M} markers from final vocabulary
\end{algorithmic}
\end{algorithm}
