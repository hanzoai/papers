...
\paragraph{4. A cross-backend \emph{correctness} finding: attention kernel choice, not just speed.}
Speed parity is not the only cross-backend variable --- numerical \emph{behaviour} is too, and it
bites hardest where the kernels differ most. Qwen3-8B-Q4\_K\_M generates coherently on Metal and
under an eager attention on CUDA, but \emph{collapses into token repetition} under CUDA
flash-attention-2 (byte-identical weights, same sampler; only the attention kernel differs). We
traced it to the reduced-precision $P\!\cdot\!V$ tensor-core matmul and, by an exhaustive
proved-by-build precision ladder (bf16 $\to$ f16 $\to$ f32-class split-$P$, each verified on the
running model), showed that raising precision only \emph{moves} the repetition attractor rather than
clearing it: the fused low-precision path and a numerically-f32 path are mutually exclusive on tensor
cores, so the stable attention \emph{is} the eager (CUDA-core, f32) kernel. The shipped fix routes
bf16 CUDA attention to eager --- coherent in production, with the only cost being long-context
prefill (short prompts and all decode are unaffected). The lesson generalizes the paper's thesis:
a multi-backend engine must hold one \emph{numerically-vetted} implementation per operation, because
``the same op, a different kernel per backend'' is a correctness hazard, not merely a perf one. (The
full bisection is paper~06, \S\,``A correctness boundary''.)
