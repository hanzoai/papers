...
\textbf{Source code.} \texttt{github.com/hanzoai/agents} (commit \texttt{TBD}); harness in \texttt{python/hanzo\_harness}, plugin loader in \texttt{packages/harness/src/plugins}.\\
\textbf{Build.} \texttt{uv sync \&\& pytest} for harness; \texttt{pnpm install \&\& pnpm test} for plugin SDK.\\
\textbf{Hardware tested.} Apple M2/M3, x86\_64 Linux. Hot-reload validated under macOS and Linux file-system event APIs.\\
\textbf{Standards referenced.} Skill.md format (Anthropic), W3C DID for plugin identity, Model Context Protocol (MCP).\\
