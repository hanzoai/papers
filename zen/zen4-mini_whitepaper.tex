...
\bottomrule
\end{tabular}
\caption{Zen4-Mini Throughput Across Hardware Configurations. Batch size is not
recorded. A single decode stream reads all 2.8 GB of INT4 weights per token, so the
4090's 1{,}008 GB/s bounds one stream at 360 tokens per second; the figures here are
aggregate across concurrent requests.}
\end{table}
