...
this is a CPU/allocation win, not a wire win on trivial payloads (the ZAP frame
is \emph{larger} there: 124 vs.\ 75 bytes), and that over the saturated 2.5\,GbE
link both transports tie at $\approx$530k. Separately, on the serialization axis
--- native typed RPC vs.\ HTTP+JSON on a shared record --- ZAP \emph{decodes}
$35$--$174\times$ faster at zero allocations (13\,ns vs.\ 473--2334\,ns), a wash
on encode, netting $1.1$--$1.3\times$ end-to-end once the socket dominates. We
release the framework as \texttt{hanzoai/benchmarks} and the tuning as a single
idempotent script.
\end{abstract}
...
array escapes to the heap. The serve path serializes with
\texttt{AppendResponse} into that reused buffer --- \textbf{zero} allocations,
versus 19 for the old fresh-slice \texttt{MarshalResponse} --- and
\texttt{UnmarshalRequest} decodes headers in place, also zero-alloc; over a warm
connection the serve loop allocates nothing per request. Golden tests confirm the
wire is
...
at $\approx$530k, saturating the wire (\S\ref{sec:ceiling}) where the codec is no
...
\section{Skipping JSON: Typed RPC vs.\ HTTP+JSON}
\label{sec:json}
Section~\ref{sec:zap} isolates \emph{framing} CPU: \texttt{zaphttp} carries the
HTTP body opaque, so both transports pay the same JSON cost and ZAP's win is the
zero-alloc frame codec alone. That leaves ZAP's larger structural claim untested
--- that a \emph{native} typed binary message is read field-by-field off the wire
with no parse step, where JSON must scan text and reflect. We test it directly:
one shared record (an \texttt{eth\_getBalance}-shaped call, four fields each way)
round-tripped by \texttt{encoding/json} (stdlib), by \texttt{goccy/go-json} and
\texttt{sonic} (the fastest third-party JSON), and by \texttt{zap-proto/go}'s
typed builder/reader. Same fields in, same compute, same fields out; a
round-trip-equivalence test enforces byte-exact re-encoding and that every field
is consumed on both sides, so neither codec skips work the other does.

The decode path is the headline (Table~\ref{tab:serde}). Reading the four-field
request costs \textbf{13\,ns and zero allocations} in ZAP --- a zero-copy read at
a fixed offset --- against 473\,ns/4\,allocs for the fastest JSON
(\texttt{goccy}) and 2334\,ns/13\,allocs for stdlib: a \textbf{35$\times$ to
174$\times$} gap, and the difference between allocating nothing and allocating
per field. This is the ``skip JSON'' advantage made concrete --- there is no
parse, only a pointer and an offset.

Two honest counterweights. \emph{Encode is a wash}, not a win: ZAP serializes in
629\,ns with \emph{more} allocations than JSON (12 vs.\ 4), because the published
\texttt{v1.3.0} builder defers variable-length tails through a copy and escapes
its object builder to the heap --- the pooled fixed-width API that removes this is
not in the released tag, so we report the tag-reproducible number. Netted out, the
server handler (decode\,+\,compute\,+\,encode) still favors ZAP at 274\,ns vs.\
\texttt{goccy}'s 431\,ns, because the decode margin dominates the encode wash.
\emph{And end-to-end the margin compresses}: over the loopback \texttt{/rpc}
endpoint ZAP serves 535k req/s against \texttt{sonic}'s 483k ($1.11\times$),
\texttt{goccy}'s 452k ($1.18\times$), and stdlib's 407k ($1.31\times$) --- real,
but far below the 174$\times$ microbenchmark gap, because at half a million
requests per second the socket and scheduler dominate and serialization is a
shrinking slice of per-request cost. Finally, on a structured record ZAP's wire is
also \emph{smaller} (162 vs.\ 233 bytes total): JSON's hex-string addresses and
decimal integers are verbose. This refines \S\ref{sec:zap} --- ZAP's frame is
larger only on trivial (\texttt{/health}-sized) payloads, and smaller once the
record carries real typed fields.

\begin{table}[t]
\centering
\caption{Serialization only, one shared 4-field record, GB10, median of 3,
default \texttt{GOGC}. Decode is where typed-binary skips JSON entirely; the
round-trip is fastest on ZAP though it allocates more than \texttt{goccy}.}
\label{tab:serde}
\begin{tabular}{lrrr}
\toprule
codec & decode (ns) & round-trip (ns) & round-trip allocs \\
\midrule
JSON stdlib          & 2334          & 3063         & 17 \\
JSON \texttt{sonic}  & 845           & 1460         & 14 \\
JSON \texttt{goccy}  & 473           & 904          & \textbf{8} \\
\textbf{ZAP typed}   & \textbf{13.4} & \textbf{630} & 12 \\
\bottomrule
\end{tabular}
\end{table}
...
and its native typed RPC decodes structured records $35$--$174\times$ faster than
JSON at zero allocations (\S\ref{sec:json}) --- a decode advantage that today is
masked end-to-end by socket overhead and a not-yet-zero-alloc encode path, and
that a pooled fixed-width builder would surface. The remaining levers to seven
figures are orthogonal and physical: a \emph{dual-loader} across the two physical
NICs (parallel RX paths), faster interconnect (10\,GbE/RDMA), and a
\texttt{writev} split so ZAP's frame tail avoids one body copy on large payloads.
We also intend to benchmark the
