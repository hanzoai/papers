...
load generator. Finally we characterize the ZAP transport: contrary to
expectation it is currently \emph{slower} than HTTP (91k vs.\ 338k req/s
loopback), and we show by construction that this is a garbage-collection
artifact of a non-zero-allocation codec (relaxing \texttt{GOGC} $100\to800$
more than doubles it to 203k). The specific defect is an un-pooled request
context in the ZAP server, not a wire property; and the ZAP wire is in fact
measurably \emph{larger} than HTTP, not smaller --- its advantage is skipping
JSON (de)serialization on typed payloads, which a trivial handler cannot show.
We release the framework as \texttt{hanzoai/benchmarks}
...
\section{The ZAP Transport: Tighter Wire, Fatter Codec}
\label{sec:zap}
...
built \texttt{zapload}, a concurrent client over \texttt{zaphttp.Transport}. The
result inverted the expectation: ZAP delivered only 91k req/s on loopback against
HTTP's 338k.

The cause is not the wire but the codec. The tail (p50 303\,\textmu s but p99
13\,ms) is a garbage-collection signature, and relaxing \texttt{GOGC} $100\to800$
more than doubled ZAP to 203k with p99 falling to 4.6\,ms
(Table~\ref{tab:zap}) --- proof that the path is allocation-bound. The
allocations live in the reference codec: \texttt{MarshalRequest} mints a fresh
frame slice per call, and header decode performs \texttt{string()} copies into a
\texttt{map[string][]string}, whereas \texttt{fasthttp}'s HTTP path is
zero-allocation. The single largest culprit is structural: \texttt{zaphttp}'s
server allocates a fresh \texttt{fasthttp.RequestCtx} per request
(\texttt{server.go}), where \texttt{fasthttp}'s own server pools it via
\texttt{sync.Pool}. Two corrections to intuition follow from
\texttt{zap-proto/bench}: the ZAP wire is \emph{larger} than HTTP (452 vs.\ 243
bytes for a small request, far worse for large bodies), and raw throughput is
comparable rather than a blowout. ZAP's real advantage is orthogonal to the wire
--- typed binary messages that skip JSON (de)serialization --- which a two-byte
handler cannot exercise. The fair comparison, now in progress in
\texttt{zap-proto/http} and \texttt{zap-proto/bench}, pools the request context
and drives a typed JSON-shaped payload, where ZAP avoids the marshal cost HTTP
pays.

\begin{table}[t]
\centering
\caption{ZAP vs.\ HTTP, loopback, \texttt{-c 125}. ZAP is GC-bound.}
\label{tab:zap}
\begin{tabular}{lrl}
\toprule
transport & req/s & note \\
\midrule
\texttt{zip} HTTP (fasthttp)   & 338--400k & zero-alloc \\
\texttt{zip} ZAP (\texttt{GOGC=100}) & 91k & alloc/GC-bound \\
...
alone, with the box only $\sim$12\% idle. The immediate levers to seven figures
are orthogonal: a \emph{zero-allocation ZAP codec} (to make the native transport
overtake HTTP), a \emph{dual-loader} across the two physical NICs (parallel RX
paths), and faster interconnect (10\,GbE/RDMA). We also intend to benchmark the
