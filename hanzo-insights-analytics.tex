...
\begin{itemize}
    ...
    \item A Datastore-backed analytical engine supporting sub-second queries over billions of events.
...
\node[box, below left=0.6cm and 0.3cm of stream] (ch) {Datastore};
...
Events are stored in a Datastore MergeTree table:
...
The dashboard displays live metrics using Server-Sent Events (SSE). The Real-Time Aggregator maintains sliding window counters in memory, flushed to Datastore every 10 seconds. Dashboard widgets include: active visitors, pageviews per minute, top pages, referrer breakdown, geographic distribution, and device/browser splits.

\section{Implementation}

Insights is implemented as a Go service (18,000 lines) with a React dashboard (12,000 lines). The collection endpoint processes events in batches of 1,000, inserting into Datastore via the native protocol for maximum throughput.

\section{Status of the performance figures}
\label{sec:figures}

An earlier version of this paper carried a table titled ``Query latency on
90-day window (ms)'' giving p50/p99 from 45/180\,ms on the dashboard overview to
200/800\,ms on cohort retention, and an abstract stating that the system
processes over 50 million events daily with sub-second query latency.

None of it was measured. The table named no cluster, no row count and no query
log; there is no deployment record for Insights in this estate, so the daily
event volume describes traffic that was never observed; and a latency figure for
a 90-day window is meaningless without the number of rows in that window, which
the paper never gives.

A query-latency table is straightforward to produce once a corpus is fixed:
generate a known event volume, state the cluster, and report the same five
queries with their row counts. That run does not exist yet.

\section{Security}

\paragraph{No PII Storage.} IP addresses are used only for visitor hashing and geolocation, then discarded. User-Agent strings are parsed into device/browser/OS categories and discarded. The raw identifiers never reach Datastore.

\paragraph{Data Isolation.} Each tenant's data is scoped by \texttt{siteId}. Datastore queries include a mandatory \texttt{siteId} predicate enforced at the API layer. Cross-tenant queries are impossible by construction.
...
Hanzo Insights demonstrates that privacy-first analytics need not sacrifice functionality. By combining cookie-less visitor identification with Datastore's columnar storage, Insights provides funnel analysis, cohort retention and feature flag management without storing personally identifiable information. Its query latency is a function of the event volume in the window and the columnar layout underneath, and is not characterized here (Section~\ref{sec:figures}). The integrated feature flag engine eliminates the need for a separate service, reducing infrastructure complexity while enabling data-driven product decisions.
