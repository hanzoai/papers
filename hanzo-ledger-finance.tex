...
\begin{table}[h]
\centering
\caption{Transaction throughput and latency. Not traceable to a recorded run; see Section~\ref{sec:figures}.}
...
hardware, no PostgreSQL configuration, no concurrency and no duration, and no
result file in \texttt{hanzoai/ledger} contains these numbers.

What distinguishes them from an invented table is that the harness which would
produce them is already in the repository: \texttt{test/performance/pkg/write}
and \texttt{test/performance/pkg/read} are a parallelism-flagged TPS harness
driven by two benchmark workflows in CI. The gap is the run and its
declaration, not the mechanism.

One claim that appeared alongside these figures is withdrawn rather than
qualified. An earlier version stated that the system maintained ``zero balance
discrepancies across 3 years of production operation.'' No reconciliation
record, incident log or monitoring export stands behind it, and there is no
three-year operating history for this ledger to observe. The invariant the
paper describes --- that debits equal credits within a transaction --- is
enforced by the engine and is a property of the code; whether it has held
across three years of live traffic is a separate, unevidenced claim.
...
Hanzo Ledger provides a financial data infrastructure that enforces the double-entry invariant at the system level, eliminating an entire class of accounting bugs. The Numscript DSL enables complex financial workflows---split payments, escrow, multi-currency conversion---to be expressed declaratively and executed atomically. Enforcing the invariant in the engine rather than in each caller is what makes that class of bug unrepresentable; Section~\ref{sec:figures} states what this paper does and does not establish about the system's behaviour in operation.
