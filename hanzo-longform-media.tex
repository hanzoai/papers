...
content-hashed unit that makes a three-hour render a resumable build DAG. We give a projection template for
wall-clock on $1$ versus $M$ nodes and label the per-shot time a projection, because no feature-length render is
measured on our stack. Fourth, \textbf{open problems} specific to film that neither companion owns: perceptual
...
consistency is a pass over the shot set. Narrative coherence --- whether the assembled film makes \emph{sense},
...
``run two thousand jobs'' into ``evaluate a build DAG,'' with all the resumability and caching that implies.
...
without the contiguity constraint because shots are interchangeable. \emph{We label $\Tshot$ a projection}: no
feature-length render is measured on our stack, and the video generator itself is a port in progress (Wan-class
MoE video~\cite{wan}) whose per-shot time is not yet a first-party number. What \emph{is} measured is every
substrate the projection rests on --- the cross-vendor ring~\cite{het}, decode parity across
backends~\cite{backend}, and the native dub's $11.6$\,fps combined throughput on a VAE-on-the-critical-path
pipeline (f16, GB10)~\cite{games} that is a shot-shaped workload in miniature --- and the structure of
Eqs.~\eqref{eq:serial}--\eqref{eq:parallel}, which holds for \emph{any} $\Tshot$ because the shots are
independent. Plug in a measured $\Tshot$ when the video port lands and the wall-clock follows; the systems claim
is the \emph{shape}, and the shape is not a projection.
