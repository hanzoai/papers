...
Formal verification is where owning the whole stack pays off. Hanzo (American-owned, Techstars~'17, founded 2014) builds both the post-quantum cryptography and the frontier AI---the \emph{Zen} model family---that depend on the properties proved here, which is what makes a machine-checked claim meaningful: there is no third-party black box whose internals are out of reach of the prover. This matters because of where trustworthy AI now comes from. America's frontier AI has narrowed to a closed two-vendor duopoly, while the broad \emph{open} ecosystem---and therefore most openly auditable frontier weights---now sits offshore. A defense or regulated operator who needs to inspect, accredit, and run a system on sovereign infrastructure is forced to choose between renting from a closed American duopoly or depending on foreign open weights. Hanzo is the American open-source alternative that owns the model and the cryptography together; the verification suite documented here is the auditable evidence that backs that claim---formal proof an accreditor can read rather than a vendor assurance they must take on faith.

% ============================================================================
\section{Lean 4 Proof Library}
% ============================================================================

The proof library resides at \texttt{hanzo/formal/lean/} and contains 50 Lean~4 files organized by domain.
...
\texttt{Consensus/Finality.lean} & End-to-end finality composition (BLS + Corona) \\
...
\item \texttt{Crypto/Corona.lean}: Lattice threshold signature unforgeability under LWE.
...
\item \textbf{Correspondence}: Each \texttt{.lean} file has a matching \texttt{.tex} paper in \texttt{hanzo/papers/proofs/} documenting the proof in traditional mathematical notation.
\end{enumerate}
...
\midrule
CB58 encoding & \texttt{hanzo/crypto/cb58/} & 2 \\
secp256k1 signatures & \texttt{hanzo/crypto/secp256k1/} & 3 \\
IPA proofs & \texttt{hanzo/crypto/ipa/} & 2 \\
BLAKE2b hashing & \texttt{hanzo/crypto/hash/blake2b/} & 1 \\
BFT consensus & \texttt{hanzo/bft/} & 4 \\
MerkleDB codec & \texttt{hanzo/node/x/merkledb/codec/} & 5 \\
Compression & \texttt{hanzo/node/utils/compression/} & 3 \\
RPC database & \texttt{hanzo/node/database/rpcdb/} & 2 \\
Deque & \texttt{hanzo/node/utils/buffer/deque/} & 1 \\
\midrule
...
\midrule
ZAP deserializer & \texttt{hanzo/zap/} & Parsing crashes, buffer overflows \\
ML-DSA sign/verify & \texttt{hanzo/crypto/mldsa/} & Malformed signature acceptance \\
ML-KEM encaps/decaps & \texttt{hanzo/crypto/mlkem/} & Malformed ciphertext crashes \\
Corona threshold & \texttt{hanzo/crypto/threshold/} & Malformed share acceptance \\
BLS aggregation & \texttt{hanzo/crypto/bls/} & Rogue key attacks \\
FHE ciphertext parsing & \texttt{hanzo/fhe/go/tfhe/} & Malformed ciphertext \\
ZKVM proof verifier & \texttt{hanzo/node/vms/zkvm/} & Invalid proof acceptance \\
RNS link protocol & \texttt{hanzo/node/network/dialer/} & Handshake state machine \\
SessionVM messages & \texttt{hanzo/session/} & Message parsing \\
\bottomrule
\end{tabular}
...
\begin{itemize}
\item \textbf{No Quasar-family TLA+ spec exists} anywhere. Amores-Sesar \etal\ (2024) provided the first formal consistency proof of Quasar, but no TLA+ model has been published.
...
Solana & -- & -- & -- & Limited \\
Avalanche & -- & -- & -- & Limited \\
\textbf{Hanzo (ours)} & \textbf{50 Lean 4} & \textbf{Planned} & \textbf{Halmos} & \textbf{23+ targets} \\
\bottomrule
\end{tabular}
...
\textbf{Source code.} \texttt{github.com/hanzoai/consensus, hanzoai/quasar} (commit \texttt{TBD}).\\
\textbf{Build.} go test ./... and rust-based fuzzers via cargo test.\\
\textbf{Hardware tested.} Linux amd64, deterministic test environment.\\
\textbf{Standards referenced.} NIST IR 8214C (Threshold Crypto), LOKI consensus fuzzing.\\
