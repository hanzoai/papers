...
\begin{abstract}
We present Hanzo Gateway, a high-performance API gateway built on the KrakenD framework that serves as the unified entry point for all Hanzo platform services. The gateway implements stateless JWT validation against Hanzo IAM, per-tenant rate limiting with sliding window counters, automatic circuit breaking for downstream failures, and request/response transformation pipelines. The gateway carries 147 endpoint definitions. This paper describes that configuration model and the middleware it drives, and reports no measurements; Section~\ref{sec:figures} records the per-request overhead and throughput figures an earlier version reported and why they are withdrawn. We describe the declarative configuration model, the plugin architecture for custom middleware, and the operational patterns that enable zero-downtime configuration reloads in production.
\end{abstract}
...
An earlier version of this paper carried a table titled ``Gateway overhead per
request (ms)'' attributing p50/p99 of 0.2/0.5\,ms to TLS and CORS, 0.3/0.8\,ms
to JWT validation, 0.05/0.1\,ms to the rate-limit check, 0.1/0.2\,ms to routing
and 0.15/0.4\,ms to response merging, totalling 0.8/2.0\,ms. The abstract gave a
median overhead of 1.2\,ms and 45{,}000 requests per second on a single node,
and the conclusion claimed under 2\,ms on the critical path.

None of it was measured. \texttt{hanzoai/gateway} contains no benchmark
function, no load generator configuration and no captured result; the table
named no hardware, no concurrency and no request count; and the abstract's
1.2\,ms median contradicted the table's own 0.8\,ms total in the same document.

A per-component breakdown of this kind is exactly what a profile produces, and
the middleware chain it describes is real. What is missing is the profile, the
load generator, and the machine --- reported the way the Hanzo cloud data plane
benchmark~\cite{clouddataplane} reports them.
...
Hanzo Gateway is a stateless API gateway whose per-request work is bounded by design: signature verification against a cached JWKS, a counter increment, and a route lookup, with no datastore on the path. The declarative configuration model enables endpoint management without code changes, while the circuit breaker and rate limiting systems protect backend services from cascading failures. What that costs per request is not established here (Section~\ref{sec:figures}). The stateless architecture enables simple horizontal scaling---adding capacity requires only deploying additional replicas.
