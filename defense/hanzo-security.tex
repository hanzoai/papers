...
defense Authorization to Operate (ATO) and a continuous-authorization posture
require.
...
cannot audit, and you certainly cannot get an ATO for, a system whose extent you
have not established; automated discovery establishes it.
...
% ============================================================================
\section{Why It Matters for Defense}
% ============================================================================

Defense and national-security systems face the estate-scale security problem in
its most acute form: larger sprawls, longer lifetimes, higher consequences, and a
formal accreditation regime that demands evidence, not assertion. The capability
described here maps directly onto those requirements.

\begin{itemize}
  \item \textbf{ATO evidence.} An Authorization to Operate requires an enumerated
  ...
  immutable audit trail produces the evidence---turning the accreditation package
  ...
  (SBOM, known-vulnerability remediation): not a one-time scan but a standing,
  re-run guarantee.
  \item \textbf{Continuous authorization.} The shift from periodic to continuous
  re-audit is exactly the shift modern frameworks (cATO, ongoing authorization)
  are pushing toward. Every closed finding becomes a standing regression check;
  every commit is re-assessed; drift is an event, not a year-end surprise. The
  authorization reflects the system as it is, not as it was at the last
  assessment.
  ...
  ledger---is owned and open. It runs offline, at the edge, in a classified
  enclave, with no dependency on a foreign or black-box service and no telemetry
  leaving the boundary. A defense customer can inspect the auditor itself, which is
  the only honest basis for trusting an auditor's output.
\end{itemize}

The deeper point the case study makes for a defense buyer is about
...
defend, accredit, and reason about. In a domain where every additional service is
additional attack surface, additional accreditation scope, and additional
...
to a security and national-security buyer. First, sovereignty of the security
...
all of it is American-owned, open, and sovereign. For defense, that is the
combination the accreditation regime is asking for: enumerate what you have, prove
