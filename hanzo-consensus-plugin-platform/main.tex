...
\twocolumn[
  ...
  one---over a fixed seven-voter quorum orders only tiny
  ...
  \emph{and} an object-store epoch compare-and-swap), the seven-voter /
  ...
  This is a design proposal (HIP-0125, Draft), not a report on a shipped
  system.
  \end{abstract}
  \vspace{1.5em}
  \end{@twocolumnfalse}
...
what we are \emph{not} claiming: this is a design (HIP-0125, Draft), the
...
Substrate & \textbf{Quasar} PQ-BFT RSM (\texttt{luxfi/consensus/protocol/quasar}), 7 voters (5-of-7 quorum) & \texttt{hanzoai/replicate} over \texttt{vfs}, async log-ship \\
...
The fleet is seven voter nodes and $N$ non-voting data nodes. With
$N \ge 3f+1$, seven is the smallest voter set whose Byzantine quorum
($2f+1 = 5$) tolerates two simultaneous faults ($f=2$);
the count is \emph{fixed} so agreement stays fast---consensus runs among seven,
...
data-plane fan-out. The defaults are seven voters and three data replicas---one
...
\emph{not} a membership change: the RSM sees the same seven voters and the same
data-node identities before and after, so no epoch reconfiguration and no
placement churn occur on a deploy, and at-most-one-unavailable keeps
six-of-seven voters live---above the five-of-seven quorum---throughout. The existing service aliases keep
...
quorum is quantum-secure on-doctrine from the start. Seven
voters plus $N$ data nodes---the smallest voter set ($N \ge 3f+1$) whose
Byzantine quorum ($2f+1 = 5$) tolerates two faults---with unbounded data-node fan-out.
...
of seven voters and three data replicas.
...
weakly-consistent byte stream, and to order only the decision. A seven-voter
