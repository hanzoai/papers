...
An earlier version of this paper carried a table titled ``Dashboard load time by
view (ms)'' giving initial and cached load times for five views, from 420/85\,ms
on the overview to 290/55\,ms on deployments. The abstract stated that Console
serves more than 1{,}200 organizations and reduces mean-time-to-diagnosis by
65\% against CLI-based workflows.

None of it was measured. The load-time table named no browser, no network
profile, no device and no sample count, and no trace or Lighthouse output exists
in \texttt{hanzoai/console}. The 65\% figure has no study behind it: no incident
dataset, no baseline definition of the CLI workflow it is compared against, and
no sample. The organization count describes an operating history the console
does not have. All three are removed.

Load time is measurable today --- a Lighthouse run against a named build on a
declared network profile answers it. Mean-time-to-diagnosis is a harder claim
and needs an incident record with both arms, which is the reason it should not
