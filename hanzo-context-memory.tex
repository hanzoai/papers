...
262K multi-hop lane \cite{pith}. Na\"ive publishes 91 / 57 pooled on a matched
gpt-4o-mini backbone \cite{naivebrain}. That page names no dataset and gives no
protocol, but it is identifiable as this benchmark from its own comparison column:
its baselines are CAR's to the decimal --- 78.0 / 30.2, and 54, 28, 18 and 7
single-hop for HippoRAG-v2, MemGPT and Cognee, Mem0 and Zep, whose multi-hop
scores it collapses into a single ``$\le$7\%''. A table is identified by its
baselines.
...
\centering
\footnotesize
\resizebox{\textwidth}{!}{\input{tables/locomo-ablation}}
...
\centering
\footnotesize
\resizebox{\textwidth}{!}{\input{tables/locomo-ablation-test}}
...
\centering
\footnotesize
\resizebox{\textwidth}{!}{\input{tables/locomo-minilm}}
...
A reader is handed the retrieved turns and scored on what it says
(Table~\ref{tab:answers}). Every row of a reader shares the prompt
(\texttt{prompts/reader-locomo.txt}), temperature 0 and a 64-token cap; rows
differ only in what was retrieved. The open reader is gemma-4-31B-it, run
locally, the reader LoCoMo-Conv fixes; qwen3.6-35B-A3B follows on all four
categories. The oracle row hands the reader exactly the annotated turns.

With qwen3.6-35B-A3B over all 1,525 questions of the four categories,
cosine context yields F1 48.8, the first cut 50.3, the engine 53.3 and the
annotated turns 61.8; exact match 26.2, 26.7, 29.0 and 34.6. Retrieval moved
the share of contexts holding a gold turn from 79.3\% to 87.0\%, and the
engine's rows are above cosine's on every category (multi-hop 36.7
$\rightarrow$ 43.0, single-hop 56.9 $\rightarrow$ 61.9, temporal 47.5
$\rightarrow$ 49.5). The multi-hop questions with Gemma say where the remaining error is. Cosine
context yields F1 37.6; the first cut 40.7; the engine 40.5; the annotated
turns themselves 53.0. Retrieval moved the share of questions whose context
holds a gold turn from 79.8\% to 87.9\%, and the reader turned that into
three points of F1 out of a fifteen-point gap to the oracle. The rest of the
gap is the reader's: a 31B open model at temperature 0, asked for the
shortest answer, scores 53.0 even when handed nothing but the evidence.
That is the ceiling any retrieval improvement is measured against on this
reader, and it is the reason the retrieval tables are reported on their own:
a reader can hide a retrieval gain, and a stronger reader raises every row.

\begin{table*}[t]
\centering
\footnotesize
\resizebox{\textwidth}{!}{\input{tables/locomo-answers}}
\caption{Answer accuracy on LoCoMo with the same reader, prompt and $k$
across rows, all ten conversations. EXACT is the share of questions whose
context held an annotated turn; ANSWERED is F1 $\geq 0.5$ or an exact match.
The oracle row hands the reader exactly the annotated turns and bounds what
retrieval can still buy. Tokens per question are the retrieved context
delivered to the reader; the reader's own p50 latency is the local Ollama
service under shared load and is reported, not claimed.}
\label{tab:answers}
\end{table*}
...
The write side parses every fact line into (serial, subject, relation,
object) — 45 templates, no line left unparsed at any size, and the gold
answer is the object of some fact for every question. The read side compiles
the question into a base entity and a chain of relations, follows the chain
hop by hop, and at each hop takes the latest serial for (entity, relation)
deterministically; the reader, where one is used, sees only the resolved
facts. The \emph{noreader} row emits the resolved object string itself, so
it is the resolver's accuracy with no model in the loop at all.

Three resolvers stand in Table~\ref{tab:mab}, each developed on the 6K
haystacks and run on the others once. The first run compiled plans with a hand-written lexicon. It scored 98.0 /
77.5 (SH / MH) on the 6K dev haystacks, held single-hop at 92.9--98.0 on
test, and fell to 32.0 / 31.3 / 25.5 on multi-hop at 32K / 64K / 262K. The
traces say why: 126 of the 209 multi-hop misses were complete chains with
the wrong answer, and 83 asked for a relation the entity never had. Both are
the plan: the lexicon emitted relations in the question's surface order
(``capital of the country that X originates from'' became
\texttt{[capital, country\_origin]} rather than the reverse) and added a hop
for every relation word it saw, and it had been written against the 6K
phrasings. Parse coverage, entity lookup and version selection were not the
problem. The second run keeps the exact single-hop templates as rules, has a
model compile every other question into the same plan shape, validated
against the relation set, and uses the lexicon only where the model's plan
fails validation. On the shared GPU every model call of that pass timed
out and the lexicon filled every plan, so its plans were the lexicon's; it
scored 59 / 59 / 33 on multi-hop at 32K / 64K / 262K, the gain over the
first run being the fixed plan policy for single-hop and a corrected
lexicon, not a model.

The third row treats the plan as a sketch. An oracle search over the
parsed graph (\texttt{mab/oracle.mjs}) found a path from the plan's base
entity to the gold for 65 of the 67 multi-hop misses at 262K; the compiler's
errors were near-synonym relations (\texttt{language} for
\texttt{official\_lang}, \texttt{origin} for \texttt{country\_origin}),
chains cut a hop short or padded with one, and the order of two hops. A
second measurement (\texttt{mab/versions.mjs}) settled, on dev, what
``current'' means: for single-hop questions the gold is the last version of
its key 71 times and the first 0; for multi-hop chains 63 are reachable with
the latest version at every hop and none with the first. So the search keeps
``latest wins'' and relaxes the plan: at each hop the planned relation is
preferred, its class-mates allowed at a cost, an older version taken only
when the latest leads nowhere, one typed bridge through the entity's own
facts when it has no such relation, a planned relation nothing satisfies
skipped at a cost, and the answer type the question names --- a city, a
country, a person, a language --- as the final constraint, deciding between
stopping a hop early and going one further. The best complete path wins by a
fixed score; ties go to the more literal plan and the later version. It is
deterministic, it reads no gold, and it calls no model at execution time.

The plans it searches over are several per question: the lexicon's, and
those of Qwen3.8-27B (4-bit, local) compiled once at temperature 0 and once
at 0.7, at the relation level and at the level of relation families, each
also in every order of its relations. Every plan of every set is executed
and the same score selects among them; \texttt{meta.json} records the
planner of every run and the frozen archive carries every plan set with its
sha256. The compiler is the one place a language model takes part, before any
haystack is read; it never sees a fact. Table~\ref{tab:mab-nbest} separates
the two: plan recall@N (some plan among the first N sets reaches the gold)
against execution success@N (the selected one does). Recall climbs with
every set and selection tracks it within a point or two --- 68 against 67
at 262K --- so the sets, not the selector, set the number. A third row asks a language model to repair a
plan the selector finds suspicious, and it is in the table at its dev numbers
and nowhere else: across 196 dev questions the repair fired \emph{once}, that
one plan was chosen, and it is the whole of the row's 88 to 89. One question,
for a 27B model held resident beside the search. It was not promoted to test,
which is why its test cells are empty --- a configuration that loses on dev does
not get a second chance on held-out data, and reporting the empty cells is
cheaper than pretending the row was never run. The fourth row
adds an exhaustive enumeration of the relaxed plan and a lexical tie-break
from the compiler's own templates, and buys five points where the repair bought
one, with no model in the loop at all. On dev it moved multi-hop from 88 to 93
and single-hop from 98 to 100; on test, run once, multi-hop 89 / 89 / 67 and
single-hop 98 / 98 / 97. Pooled, 98.3 / 84.5, counting every unanswered
question as wrong.

Against the reference points: CAR's gpt-4o pipeline reports 94.8 / 51.5
pooled and 93 / 41 at 262K; this search reports 98.3 / 84.5 pooled and
97 / 67 at 262K with no reader. Na\"ive's 91 / 57 is pooled on the same
benchmark, identified as above, on a matched gpt-4o-mini backbone; this search
reports 98.3 / 84.5 with no reader at all, +7.3 single-hop and +27.5 multi-hop.
The per-lane split is not published there, so the comparison is of pooled figures,
and what varied under their measurement is unstated. Pith
reports multi-hop 95 / 83 / 84 / 68 at 6K / 32K / 64K / 262K under its own
protocol \cite{pith}, a four-lane mean of 82.5 against this search's 84.5:
above it at 32K and 64K, two below at 6K and one below at 262K, inside the
bootstrap interval on every lane. Its protocol is undisclosed, so this
compares reported numbers, not systems, and we claim no best published
result on the benchmark.

\paragraph{What stops 262K at two thirds.} The search's 33 misses at 262K
are 29 questions whose gold uses an \emph{older} version of a key that a
later line updates, and 4 where a shortcut path hides the same choice;
every one of the 100 has a gold path in the parsed store
(Table~\ref{tab:mab-ceiling}). The later line is an edit no question in the
haystack asks for: the benchmark is built from MQuAKE-CF counterfactual
edits, a haystack packs the edits of instances it never asks about, and the
gold follows the edit on 183 versioned hops and the original on 46. Two
ceilings follow. A key is contested when two questions need it in different
versions; a policy choosing one version per key, however it chooses, can
satisfy one side of each contest, which caps an oracle at 94 at 262K.
``Latest wins'' caps at 67, and the search is on that line. Six signals a
store can read were tried against the 46 older-gold hops, with the 183
latest-gold hops as control: the object's degree (gold higher 21, lower
20), whether the object has facts of its own (equal), whether it is tied
back to its subject elsewhere (13 to 0 for the gold --- and 4 to 42 on the
control, because the tie marks the \emph{original} fact, which is the gold
one time in five), contest by another question (6 questions), serial
distance to the chain's other facts (24 to 38), and position in the list by
decile (one distribution for followed and ignored edits). None separates
them. A system that reads only the haystack cannot pass 67 at 262K without
guessing; the published 68 is one question above that line. The residue is
a property of the benchmark at that size, and the way past it is not a
reader but the question's own instance --- which the haystack does not
carry.
...
\footnotesize
\input{tables/mab-fc}
\caption{Substring exact match on FactConsolidation. Rows name their
reader; \emph{none} is the resolver's own output. The 6K columns are the dev
split; 32K--262K were run once per configuration. Pooled is the mean of the
four sizes, as CAR reports its headline.}
\label{tab:mab}
\end{table*}
...
\centering
\footnotesize
\input{tables/mab-nbest}
\caption{Best-of-N against selected-of-N on the multi-hop haystacks: plan
recall@N / execution success@N out of 100, no reader. Sets enter in the
lane's order; every set is also executed in every order of its relations.}
\label{tab:mab-nbest}
\end{table}
...
\centering
\footnotesize
\input{tables/mab-ceiling}
\caption{The ceiling of a consistent version policy per multi-hop haystack,
out of 100. A contested key is one that two questions need in different
versions; the oracle row picks one version per key to satisfy the most
questions, the latest-wins row is the policy every row of
Table~\ref{tab:mab} uses.}
\label{tab:mab-ceiling}
\end{table}
...
\bibitem{naivebrain} Na\"ive. Agent Brain benchmark results.
\url{usenaive.ai/lab/memory}, dated July 2026, read 15 September 2026. The page
reports 91 / 57 on ``FictionalCharacters QA on a matched gpt-4o-mini backbone'',
metric ``\% of held-out questions answered correctly'', with ``Na\"ive measured,
others published''. No dataset source, question count, protocol or citation is
given; a preprint, ``Both Engines, One Harness'', is announced as forthcoming.
