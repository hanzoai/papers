...
\begin{description}[leftmargin=1.2em,style=nextline]
  ...
  \item[Compute on ciphertext (F-Chain FHE, Z-Chain rollups).] To answer
  queries without decrypting, predicates and aggregates evaluate homomorphically
  against the encrypted pages. This is not hypothetical: the FHE compute is a
  shipping lattice-based library (\texttt{luxfi/fhe}, with TFHE bootstrapping)
  exposed as the F-Chain, and the FHE-CRDT combination --- homomorphic operations
  that also merge conflict-free --- already exists. Because FHE evaluation is
  expensive, it is scaled and made publicly verifiable by Z-Chain ZK rollups:
  many encrypted operations are batched off-chain and a single succinct proof,
  posted to the rollup, attests that the batch was evaluated correctly over the
  committed ciphertext. The server thus returns an encrypted result the client
  alone can open, with a rollup proof that it was computed honestly --- a public,
  permissionless confidential-compute path that needs no trusted server.
...
custodies the post-quantum keys (KEM). \textbf{F-Chain} runs the FHE compute
(\texttt{luxfi/fhe} / TFHE), and \textbf{Z-Chain} provides the ZK rollups that
scale and publicly verify it. \textbf{Quasar} provides post-quantum, leaderless
finality for the commitments of both engines. And \textbf{ZAP} --- the
Cap'n-Proto-derived transport that already replaced gRPC --- is the single wire
for client-to-engine queries, engine-to-S3 part transfer, and
engine-to-consensus coordination, so the entire platform is one transport deep
rather than the usual stack of HTTP, gRPC, and a ZooKeeper wire.

\paragraph{Post-quantum end to end.} A property worth stating explicitly because
it holds at \emph{every} layer, not one: the consensus finalizes with ML-DSA
(Quasar), the per-user data keys are post-quantum KEM (K-Chain), the
classical-primitive precompiles are gated behind a post-quantum profile (P3Q),
and the confidential compute is lattice-based FHE (F-Chain). There is no
classical-cryptography weak link in the path from a client's key to a finalized
commitment. The endpoint of the program in this paper is therefore a platform
that is, simultaneously, one transport deep (no internal HTTP, no gRPC, no
ZooKeeper --- only ZAP and Quasar) and post-quantum native end to end, over
decentralized S-Chain storage.
