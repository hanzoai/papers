...
The first external lane is measured; the deterministic benchmark of
Section~\ref{sec:lsp} is still pending and its table is left as such.

\paragraph{RepoBench-R.} Python, both settings, the dev split
(\texttt{train\_easy[0:100] + train\_hard[0:100]}) for every weight and the
test split (\texttt{test\_easy[0:250] + test\_hard[0:250]}) run once at the
frozen commit. Each item is a completion point with about twelve candidate
snippets from other files of the repository, one of which the next line
needs. The dense generator embeds the last lines above the point and each
candidate with all-MiniLM-L6-v2; BM25 is queried with the last three lines;
the typed links are regular expressions over \texttt{import}, \texttt{from
\ldots\ import}, \texttt{def}, \texttt{class} and identifiers — a candidate
that defines an imported name the code above has not used yet, or has. No
model reads anything and nothing reads the answer.

Table~\ref{tab:repobench} is the memory result again in another domain.
Dense similarity alone finds the right snippet first 18.8\% of the time in
the cross-file-first setting; the typed links alone, with no model, 31.4\%;
the two together 33.2\%, with MRR 40.8 $\rightarrow$ 52.3 and R@5 72.8
$\rightarrow$ 79.4. On the hard items, where the file's own context gives
the least away, dense retrieval finds the snippet first 12.8\% of the time
and the links 25.6\%. In the cross-file-random setting the module has
already been used above the point, the ``unused import'' prior no longer
applies, and the links' margin narrows: 24.6 $\rightarrow$ 31.2 R@1 with
dense plus links. Retrieval takes under a millisecond per item. The sign of
the ``used'' term flips between the two settings, and was chosen on dev for
each; that is the one setting-specific decision in the lane, and the sweep
table records it.

\begin{table*}[t]
\centering
\footnotesize
\input{tables/code-repobench}
\caption{RepoBench-R Python, test split, retrieval only, one row per
configuration. R@$k$ is whether the gold snippet is in the top $k$ of about
twelve candidates; \emph{hard} is the benchmark's own level. Weights frozen
on dev; test run once.}
\label{tab:repobench}
\end{table*}
...
\midrule
RepoBench-R (retrieval, Table~\ref{tab:repobench}) & 52.3 MRR & --- & --- \\
