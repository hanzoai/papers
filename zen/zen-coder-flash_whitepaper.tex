...
Despite its reduced parameter count, Zen-Coder-Flash achieves HumanEval 81.4\%---93.4\% of
...
\begin{table}[H]
\centering
\caption{Generation throughput (tokens/second) on single A100-80GB. The Batch=1
column exceeds what one stream can read: BF16 weights are 16.4 GB for the 8B model and
28.8 GB for the 14B, and the A100's 2{,}039 GB/s bounds single-stream decode at 124 and
71 tokens per second respectively. The batched columns carry no such conflict.}
