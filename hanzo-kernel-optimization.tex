...
split $P_{hi}{+}P_{mid}{+}P_{lo}$ & $\sim$33 ($>$ f32) & \texttt{exp2f} & \texttt{"PostScriptScript\dots"} \\
...
\paragraph{The boundary --- and the decisive over-shoot.} To settle whether the lone remaining
variable was $P$ precision (the tensor-core path's $\sim$20-bit vs eager's 23-bit f32), we pushed a
\emph{three}-term split $P_{hi}{+}P_{mid}{+}P_{lo}$ to $\sim$\textbf{33 mantissa bits --- beyond full
f32}, more $P$ precision than eager itself carries. It \emph{still} degenerates (into a fresh
\texttt{"PostScript\dots"} attractor). This is conclusive: raising $P$ precision \emph{past eager's
own} does not cure it, so the collapse is \textbf{not a $P$-precision phenomenon at all}. With $P$
over-shot, $V$ identical (bf16) in both paths, scores f32-identical, single-tile (no rescale), and
\texttt{exp} eliminated, the sole surviving difference is the \emph{fused tiled computation venue}
itself: the flash structure converges to a different, degenerate numerical fixpoint than eager's
single materialized pass, and \emph{no precision knob closes it}. The stable attention \emph{is} the
eager (materialized, single-pass) computation --- not because tensor cores lack f32, but because the
fused structure is inherently on the wrong side of this model's repetition bifurcation. The shipped
fix routes bf16 attention to eager; it is not a stopgap but the \emph{only} stable structure, and it
is essentially optimal on speed as well: any fused stable variant would have to abandon the tiled
tensor-core venue that causes the collapse. Its only cost is
