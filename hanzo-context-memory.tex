...
The third resolver treats the plan as a sketch. An oracle search over the
parsed graph (\texttt{mab/oracle.mjs}) found a path from the plan's base
entity to the gold for 65 of the 67 multi-hop misses at 262K, and a ceiling
of 98 for a perfect planner; the compiler's errors were near-synonym
relations (\texttt{language} for \texttt{official\_lang}, \texttt{origin}
for \texttt{country\_origin}), chains cut a hop short or padded with one,
and the order of two hops. A second measurement (\texttt{mab/versions.mjs})
settled, on dev, what ``current'' means: for single-hop questions the gold is
the last version of its key 71 times and the first 0; for multi-hop chains
63 are reachable with the latest version at every hop and none with the
first. So the search keeps ``latest wins'' and relaxes the plan: at each hop
the planned relation is preferred, its class-mates allowed at a cost, an
older version taken only when the latest leads nowhere, one typed bridge
through the entity's own facts when it has no such relation, a planned
relation nothing satisfies skipped at a cost, and the answer type the
question names — a city, a country, a person, a language — as the final
constraint, deciding between stopping a hop early and going one further.
The best complete path wins by a fixed score; ties go to the more literal
plan and the later version. It is deterministic, it reads no gold, and it
calls no model. In the runs of Table~\ref{tab:mab} the plans it searched
over were the lexicon's (\texttt{meta.json} records the planner of every
run), so no language model took part at any point; model-compiled plan sets
are the next rows, labelled as such. On dev it moved multi-hop from 76 to 83 and single-hop
from 96 to 98; on test, run once, multi-hop 76 / 76 / 51 and single-hop
97 / 97 / 94. Pooled, 96.5 / 71.5, counting every unanswered question as
wrong.

Against the reference points: CAR's gpt-4o pipeline reports 94.8 / 51.5
pooled and 93 / 41 at 262K; this resolver reports 96.5 / 71.5 pooled and
94 / 51 at 262K with no reader and, in these runs, no compiler model either. The unsourced ``91 / 57'' is exceeded
on both columns pooled, and we make no claim against it beyond that. Pith
reports multi-hop 95 / 83 / 84 / 68 at 6K / 32K / 64K / 262K under its own
protocol \cite{pith}, a four-lane mean near 82.5; this resolver is below it
on every lane, so it is not the best published result on this benchmark, and
the 262K column is where the remaining error is: \texttt{mab/oracle.mjs} puts
its ceiling at 98 with the plan the only thing standing between 51 and it — the compiler's
chain shape on the longest questions, which the search cannot repair when
the plan names a hop that does not exist and omits one that does. The next
step is several plans per question, executed deterministically and chosen
by the same score, not a reader.
